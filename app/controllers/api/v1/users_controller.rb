module Api
  module V1
    class UsersController < ApiController
      ...
      before_action -> { authorize_user [UserRole::Admin] }, only: %i[index show]
      ...
      def init_user(id = params[:id])
        @user = User.accessible(Session.user).where(id: id).first
      ...
      def user_params
        valid_params = params.permit(:login, :email, :role_code, :status_code, :first_name, :last_name)
        valid_params
      end
    end
  end
